  ...
  return (
    <div className="min-h-screen w-full text-slate-100 bg-[radial-gradient(1200px_700px_at_80%_-10%,#16202c_0%,#0b111a_55%)]">
      <div className="mx-auto max-w-md px-4 pb-24 pt-5 flex flex-col items-center">
        ...
        <div
          ref={tapCardRef}
          className="relative mt-4 w-full rounded-3xl border border-white/10 bg-white/[0.03] p-4 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.03)] overflow-hidden"
        >
          <img
            ref={mascotRef}
            src="/mr-t.png"
          ...
          {showShop && (
            <Card>
              {shop.map((i) => (
                <div
                  key={i.id}
                  className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 p-3 mb-2"
                >
                  <div>
                  ...
                  <button
                    onClick={() => buy(i)}
                    className="rounded-xl bg-sky-500 px-3 py-1 text-sm font-semibold text-slate-950"
                  >
                    Buy
                  </button>
                </div>
...
const Quest = ({ title, reward, onClaim }: { title: string; reward: number; onClaim: () => void }) => {
    ...
    <div className="flex items-center justify-between py-3">
      ...
      <button
        ...
        className={`rounded-lg px-3 py-1 text-xs font-semibold ${
          claimed ? "bg-white/10 text-slate-400" : "bg-emerald-500/20 text-emerald-200"
        }`}
      >
...
function useInterval(cb: () => void, ms: number) {
  ...
  useEffect(() => {
    const id = setInterval(() => s.current(), ms);
...
const toast = (m: string) =>
  window.Telegram?.WebApp?.showPopup
    ...
    : console.log(m);
const lsGetNumber = (k: string, f: number) => {
  try {
