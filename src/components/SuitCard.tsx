  ...
  return (
    <div className="rounded-2xl p-3 bg-zinc-900/60 border border-zinc-700 flex flex-col items-center gap-2">
      ...
      <div className="text-center">
        ...
        <div className="text-xs text-emerald-300">{suit.bonus}</div>
        <div className="text-[11px] text-zinc-400">Unlock: {suit.unlock}</div>
      </div>
      {!owned ? (
        <button onClick={markOwned} className="mt-1 px-3 py-1 rounded-lg bg-emerald-600 text-white text-sm">Mark as Owned</button>
      ) : (
        <button onClick={equip} className={`mt-1 px-3 py-1 rounded-lg text-sm ${equipped ? 'bg-zinc-700 text-white' : 'bg-emerald-600 text-white'}`}>
