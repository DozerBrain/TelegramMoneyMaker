...
export function getLastDrop<T = any>(): T | null {
  return get(KEY_LAST_DROP, null as T | null);
}
...
export function setOwnedPets(ids: string[]) {
  set(KEYS.ownedPets, Array.from(new Set(ids)));
}
...
export function setOwnedSuits(ids: string[]) {
  set(KEYS.ownedSuits, Array.from(new Set(ids)));
}
export function getEquippedSuit(): string | null {
  return get(KEYS.equippedSuit, null as string | null);
}
export function setEquippedSuit(id: string | null) {
  set(KEYS.equippedSuit, id);
}

// -------- 👤 PROFILE (used by some pages/components) ----------
type PlayerProfile = {
  username?: string;
  region?: string;
  avatar?: string;
...
export function getProfile(): PlayerProfile {
  return get(KEY_PROFILE, { username: 'Player', region: 'World' });
}
...
// ===================================================================================
// ✅ Missing exports other files expect: defaultSave, loadSave, saveSave, StorageAPI
// ===================================================================================
...
/**
 * Build a SaveData snapshot from the individual keys we already use.
 */
export function loadSave(): SaveData {
  return {
    ...
    lastDrop: getLastDrop(),
    ownedPets: getOwnedPets(),
    equippedPet: getEquippedPet(),
    ownedSuits: getOwnedSuits(),
    equippedSuit: getEquippedSuit(),
    profile: getProfile(),
  };
}

/**
 * Persist a SaveData snapshot back into the individual keys we already use.
 * Also stores a copy under KEY_SAVE_SNAPSHOT for debugging/portability.
 */
export function saveSave(data: SaveData): void {
  setScore(data.score);
  setTap(data.tap);
  setCollection(data.collection);
  setLastDrop(data.lastDrop);
  setOwnedPets(data.ownedPets);
  setEquippedPet(data.equippedPet);
  setOwnedSuits(data.ownedSuits);
  setEquippedSuit(data.equippedSuit);
  setProfile(data.profile);
  set(KEY_SAVE_SNAPSHOT, data);
}

/**
 * Convenience object (some files import `StorageAPI`).
 */
